...
class ESSAHaitiEducationEnvironment:
    ...
    def observe(self) -> ESSAHaitiCondition:
        gang_pattern = ("high", "high", "extreme", "high", "medium", "extreme")
        group_violence_pattern = (
            "armed_presence",
            "kidnapping_reports",
            "crossfire",
            "forced_recruitment_risk",
            "intimidation",
            "armed_attack",
        )
        ...
        return ESSAHaitiCondition(
            turn=self.turn,
            gang_pressure=gang_pattern[i],
            group_violence=group_violence_pattern[i],
    ...
    def apply(self, action: str) -> ESSAHaitiResult:
    ...
    def _hidden_effect(
        self, action: str, condition: ESSAHaitiCondition
    ) -> dict[str, float]:
        route_danger = condition.route_risk in {"unsafe", "crossfire", "kidnap_risk"}
        group_violence_active = condition.group_violence in {
            "kidnapping_reports",
            "crossfire",
            "forced_recruitment_risk",
            "armed_attack",
        ...
        if action == "secure_learning_route":
            if route_danger or group_violence_active:
...
class ESSAHaitiEducationLearner:
    """ESSA learns child-safety intervention rules from a Haiti education case."""
    ...
    def __init__(
        ...
        self.previous_condition: ESSAHaitiCondition | None = None
        self.cycle_number = 0
        self.self_model.inspect_substrate(
            StaticSubstrateInspector(
                SubstrateSnapshot(
                    id="essa-haiti-education-case",
                    ...
                    attributes={
                        "actions": list(ESSA_HAITI_ACTIONS),
    ...
    def _observe(self, condition: ESSAHaitiCondition) -> Observation:
        return self.self_model.observe_world(
            "essa_haiti_education_environment",
            {
                "turn": condition.turn,
                "gang_pressure": condition.gang_pressure,
                "group_violence": condition.group_violence,
    ...
    def _detect(self, condition: ESSAHaitiCondition) -> dict[str, Any]:
        group_violence_active = condition.group_violence in {
            "kidnapping_reports",
            "crossfire",
            "forced_recruitment_risk",
        ...
        features = {
            ...
            if condition.gang_pressure in {"high", "extreme"} or group_violence_active
            else "medium",
            "group_violence": "active" if group_violence_active else "present",
            "route_access": "danger"
    ...
    def _predict(
        ...
        return {
            ...
            "written_prediction": (
                f"If ESSA performs {chosen['action']}, child-safety uncertainty "
    ...
    def _evaluate(
        self, prediction: dict[str, Any], result: ESSAHaitiResult
    ) -> dict[str, Any]:
    ...
    def _update(
        ...
        self.self_model.observe_self(
            ...
            evidence="ESSAHaitiEducationLearner._update",
        )
        return {
    ...
    def _surprises(
        self, condition: ESSAHaitiCondition, features: dict[str, str]
    ) -> list[str]:
        ...
        if (
            features["group_violence"] == "active"
            and condition.felt_safety >= self.previous_condition.felt_safety
        ):
            surprises.append("group_violence_active_but_felt_safety_not_lower")
        return surprises
    ...
    def _beliefs_for(self, action: str, result: ESSAHaitiResult) -> list[Belief]:
        ids = []
        if action == "secure_learning_route":
            ids.append("B1")
        if action == "provide_meals_and_supplies":
...
EmmaCondition = ESSAHaitiCondition
EmmaResult = ESSAHaitiResult
EmmaHaitiEducationEnvironment = ESSAHaitiEducationEnvironment
EmmaHaitiEducationLearner = ESSAHaitiEducationLearner
